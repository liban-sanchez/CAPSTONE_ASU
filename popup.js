...
const imagesOnlyToggle = document.getElementById("images-only-toggle"); // Added
...
focusMode.addEventListener("change", (e) => {
  ...
  chrome.storage.sync.set({ isFocusMode: isFocusMode });
});

restrictDomain.addEventListener("change", (e) => {
  isRestrictDomain = Boolean(e.target.checked);
  chrome.storage.sync.set({ isRestrictDomain: isRestrictDomain });
});

// Added event listener for images only
if (imagesOnlyToggle) {
  imagesOnlyToggle.addEventListener("change", (e) => {
    isImagesOnly = Boolean(e.target.checked);
    chrome.storage.sync.set({ isImagesOnly: isImagesOnly });
  });
}
...
function fillOptions() {
  chrome.storage.sync.get((items) => {
    ...
    isImagesOnly = items.isImagesOnly ?? false; // Added lookup
    ...
    if (imagesOnlyToggle) { // Added fill logic
      imagesOnlyToggle.checked = Boolean(isImagesOnly);
    }
  });
}
