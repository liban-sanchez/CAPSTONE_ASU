...
let maxDepthValue = 0;
let isImagesOnly = false; // Added state
...
broadcastChannel.addEventListener("message", (event) => {
  ...
  selectedManualLinks = Array.isArray(selectedManualLinks)
    ...
    : [];
  isImagesOnly = Boolean(isImagesOnly); // Added normalizer
...
async function processCSSImages(cssData, cssUrl) {
  ...
  const updatedCSS = cssData.replace(imageUrlRegex, (match, imageUrl) => {
    ...
    // Use getSafeFilename to handle query strings, length limits, and illegal characters
    let imageName = getSafeFilename(resolvedUrl, "png");
...
async function processImages(htmlData, inputUrl, isRootPage = false) {
  ...
  for (let imgElement of imgElements) {
    try {
      ...
      // 1. Handle Base64 Data URIs (Crucial for Google Images)
      if (imgSrc.startsWith("data:image/")) {
        try {
          // Extract the mime type (e.g., image/jpeg) and the base64 string
          let mimeType = imgSrc.split(';')[0].split(':')[1];
          let ext = mimeType.split('/')[1] || 'png';
          let base64Data = imgSrc.split(',')[1];
          
          inlineImageCounter++;
          let imageName = `inline_image_${inlineImageCounter}.${ext}`;

          if (!urlImages.includes(imageName)) {
            urlImages.push(imageName);
            // JSZip has native support for saving base64 strings as binary files
            zip.file("img/" + imageName, base64Data, { base64: true });
          }
        ...
        // Skip the standard HTTP fetch logic below since we just saved it
        continue; 
      }

      // Skip any other non-image data URIs
      if (imgSrc.startsWith("data:")) continue;

      // 2. Standard HTTP/HTTPS image handling
      ...
      // Fix for the missing extension issue (forces .png if no extension is found)
...
async function processHTML(inputUrl, html = "", savedPagesMap = null, isRootPage = false) {
  ...
  // Always download standard page images
  htmlData = await processImages(htmlData, inputUrl, isRootPage);

  // Skip CSS, JS, PDF, and Videos entirely when Images Only is turned on
  if (!isImagesOnly) {
...
async function processLinks() {
  if (maxDepthValue == 0) {
    updateStatus(isImagesOnly ? "Fetching images..." : "Saving current page...");
    const html = await processHTML(currentPage);
    
    // Added logic to skip zipping the HTML itself
    if (!isImagesOnly) {
  ...
  if (maxDepthValue == 1) {
    ...
    for (let url of urls) {
      try {
        updateStatus(
          depthOneMode === "manual"
            ? "Foreground crawl is running. Scanning selected linked pages." // changed status wording
            : "Foreground crawl is running. Scanning linked pages..." 
        );
...
async function startScrapingProcess() {
  ...
  try {
    ...
    const downloadId = await chrome.downloads.download({
      url: urlBlob,
      filename: zipName + (isImagesOnly ? "_images.zip" : ".zip"), // added context to filename
      saveAs: true,
    });
...
/**
 * Extracts a safe filename from a URL by removing illegal OS characters,
 * query parameters, and keeping file length within Windows limits.
 */
function getSafeFilename(url, defaultExt = "file") {
  if (!url || typeof url !== "string") {
    return "asset_" + Math.floor(Math.random() * 100000) + "." + defaultExt;
  }

  // Strip query parameters and hashes before extracting the name
  let cleanUrl = url.split("?")[0].split("#")[0];

  // Isolate the base filename from path
  let filename = cleanUrl.substring(cleanUrl.lastIndexOf("/") + 1);

  // Remove all reserved OS characters: \ / : * ? " < > | =
  filename = filename.replace(/[^a-zA-Z0-9.\-_]/g, "_");

  // Truncate long filenames to avoid exceeding MAX_PATH limits
  if (filename.length > 80) {
    let parts = filename.split(".");
    let ext = parts.length > 1 ? parts.pop() : defaultExt;
    let name = parts.join("_").substring(0, 70);
    filename = `${name}.${ext}`;
  }

  // Fallback if filename becomes empty after sanitization
  if (!filename || filename === `.${defaultExt}`) {
